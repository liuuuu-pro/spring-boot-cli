 ...
 */
public class OrderQueryCancelWorker implements IWorker<Long, String>, ICallback<Long, String> {

    @Override
    public void begin() {
        System.out.println("Worker开始查询cancel状态的订单...");
    }

    @Override
    public void result(boolean success, Long orderId, WorkResult<String> result) {
        System.out.println(String.format("cancel状态的订单查询完毕回调结果:success:%s, orderId:%s, result:%s",
    ...
    public String action(Long orderId, Map<String, WorkerWrapper> map) {
        ...
        System.out.println(String.format("cancel状态的订单查询完毕.OrderId:%s", orderId));
        return "cancel";
    }
